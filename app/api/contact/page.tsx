...
export default function ContactPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (cancelled || !tsContainerRef.current || !siteKey) return;

        // 既存ウィジェットがあればリセット
        if (widgetIdRef.current && (window as any).turnstile?.reset) {
          (window as any).turnstile.reset(widgetIdRef.current);
          widgetIdRef.current = null; // ← reset() は void。代入しない
        }

        // ウィジェット描画（型は types/turnstile.d.ts に集約済み）
        const id = (window as any).turnstile?.render?.(tsContainerRef.current, {
          ...
          // 必要なら "error-callback" や "expired-callback" も指定可
        });
  ...
  const track = (name: string, data?: Record<string, any>) =>
    (typeof window !== "undefined" && (window as any)?.umami?.track?.(name, data));
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // 入力リセット
      setName(""); setCompany(""); setEmail(""); setTel(""); setMessage("");
      setToken(null);

      // ウィジェットをリセット（戻り値は void）
        ...
        <Card className="shadow-lg">
          <CardHeader className="space-y-2">
            ...
            <p className="text-sm text-muted-foreground">
              フォーム送信後、受付メールを自動送信します（1分以内）。未着の場合は迷惑メールをご確認ください。
            </p>
          </CardHeader>
