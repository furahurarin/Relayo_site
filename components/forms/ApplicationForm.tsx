...
const Schema = z.object({
  ...
  agree: z.literal(true, { errorMap: () => ({ message: "規約とプライバシーに同意が必要です" }) }),
...
export default function ApplicationForm() {
    ...
    const t = setInterval(() => {
      if (window.turnstile && widgetRef.current) {
        window.turnstile.render(widgetRef.current, {
          ...
          "error-callback": () => setError("認証エラーが発生しました。ページを再読み込みしてください。"),
        ...
        clearInterval(t);
        setError("認証スクリプトの読み込みに失敗しました。少し時間をおいてお試しください。");
      }
    }, 200);
  ...
  return (
    <Card className="max-w-2xl mx-auto">
      ...
      <CardContent>
        <form className="space-y-6" action={async (fd) => onSubmit(fd)}>
          ...
          <div>
            ...
            <textarea name="detail" required rows={6} className="w-full rounded-md border px-3 py-2" />
          </div>
          ...
          <label className="flex items-start gap-2 text-sm">
            ...
            <span>
              <a className="underline" href="/legal/terms" target="_blank" rel="noreferrer">利用規約</a> と{" "}
              <a className="underline" href="/legal/privacy" target="_blank" rel="noreferrer">プライバシー</a> に同意します
            </span>
          </label>
