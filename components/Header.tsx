...
const NAV: NavItem[] = [
  { href: "/", label: "ホーム" },
  { href: "/pricing", label: "料金" }, // ★ 追加：料金ページ
  // { href: "/services", label: "サービス" }, // ←公開済みなら有効化
...
export default function Header() {
  ...
  const isActive = (href: string) => {
    // 完全一致（トップ含む）。必要なら startsWith に変更可
            ...
            {NAV.map((n) => {
              ...
              return (
                <Link
                  ...
                  href={n.href}
                  aria-current={active ? "page" : undefined}
                  className={`rounded-md px-2 py-2 ${active ? "bg-gray-100 dark:bg-gray-900 font-semibold" : "text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-900"}`}
